...
import { ChevronLeft, LogOut, Star, BookOpen, ShieldCheck, Edit2, MessageSquarePlus, UserX, MapPin, Loader, CheckCircle, X } from 'lucide-react'
...
export default function Profile({ isActive, onBack, onOpenBook, onEditBook }) {
  ...
  async function fetchBlockedUsers() {
    ...
    const { data } = await supabase.from('blocks').select('id, blocked_id, blocked:blocked_id(full_name, college)').eq('blocker_id', user.id).order('created_at', { ascending: false })
  ...
  async function handlePinChange(val) {
    ...
    if (cleaned.length === 6) {
      ...
      else setPinError('Could not find this PIN code')
    }
  }
    ...
    return (
      <div className="min-h-screen bg-cream pb-24">
        ...
        <div className="px-4 pt-5">
          {blockedLoading && <div className="flex flex-col gap-3">{[1,2].map(i => <div key={i} className="h-16 bg-white rounded-card border border-border animate-pulse" />)}</div>}
          ...
          <div className="flex flex-col gap-3">
            {blockedUsers.map((b) => (
              <div key={b.id} className="bg-white rounded-card border border-border p-4 flex items-center gap-3">
                ...
                <button onClick={() => handleUnblock(b.id)} className="shrink-0 text-xs font-display font-bold text-orange border border-orange px-3 py-1.5 rounded-chip active:scale-95 transition-transform">
                  Unblock
                </button>
              </div>
            ))}
    ...
    <div className="min-h-screen bg-cream pb-24">
      ...
      <div className="bg-navy px-4 pt-5 pb-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="font-display font-bold text-white text-lg leading-tight truncate">{profile?.full_name || 'Your Name'}</h2>
            <p className="text-white/60 text-xs mt-0.5 truncate">{profile?.college || 'College not set'}</p>
            {/* City with edit */}
            <button onClick={openLocationEdit} className="flex items-center gap-1 mt-1">
              <MapPin size={11} className="text-orange shrink-0" />
              <span className="text-orange text-xs font-medium">{profile?.city || 'Add your city'}</span>
              <Edit2 size={10} className="text-white/40 ml-1" />
            </button>
          </div>
        </div>
      ...
      {/* Location edit modal */}
      {editingLocation && (
        <div className="fixed inset-0 bg-navy/60 backdrop-blur-sm z-50 flex items-end justify-center" onClick={(e) => e.target === e.currentTarget && setEditingLocation(false)}>
          <div className="bg-white w-full max-w-lg rounded-t-[28px] px-5 pt-5 pb-10 shadow-2xl">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setEditingLocation(false)} className="text-muted"><X size={20} /></button>
            </div>
            <div className="flex flex-col gap-4">
              <div>
                <label className="block text-xs font-semibold text-navy mb-1.5 font-display">City / Area</label>
                <input type="text" value={newCity} onChange={(e) => setNewCity(e.target.value)} placeholder="e.g. Mumbai, Pune"
              ...
              <div>
                <label className="block text-xs font-semibold text-navy mb-1.5 font-display">
                  PIN Code <span className="text-muted font-normal">(optional — for distance filtering)</span>
                </label>
                <div className="relative">
                  <input type="text" inputMode="numeric" value={newPin} onChange={(e) => handlePinChange(e.target.value)} placeholder="e.g. 400019" maxLength={6}
                    className={`w-full bg-cream border rounded-button px-4 py-3 text-sm text-navy outline-none transition-colors pr-10 ${pinError ? 'border-danger' : pinResolved ? 'border-success' : 'border-border focus:border-orange'}`} />
                ...
                {pinResolved && <p className="text-[11px] text-success mt-1">✓ {pinResolved.display}</p>}
              </div>
              <Button variant="primary" size="lg" className="w-full" onClick={saveLocation} disabled={locationSaving || !newCity.trim()}>
      ...
      <div className="px-4 pt-4">
        {loading && <div className="flex flex-col gap-3">{[1,2].map(i => <div key={i} className="h-20 bg-white rounded-card border border-border animate-pulse" />)}</div>}
        ...
        {!loading && tab === 'Sold' && (
          sold.length === 0 ? (
            ...
            <div className="flex flex-col gap-3">
              {sold.map((book) => (
                <div key={book.id} onClick={() => onOpenBook?.(book)} className="bg-white rounded-card border border-border p-3 flex items-center gap-3 cursor-pointer active:scale-[0.98] transition-transform">
              ...
              {ratings.map((r) => (
                <div key={r.id} className="bg-white rounded-card border border-border p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex">
                      {[1,2,3,4,5].map((s) => <Star key={s} size={13} className={s <= r.stars ? 'text-warning fill-warning' : 'text-border fill-border'} />)}
                    </div>
                  </div>
        ...
        <div className="mt-8 flex flex-col gap-2.5 mb-4">
          <button onClick={() => setShowFeedback(true)} className="w-full flex items-center gap-3 bg-white border border-border rounded-card px-4 py-4 text-left active:scale-[0.98] transition-transform">
            <div className="w-9 h-9 rounded-xl bg-orange/10 flex items-center justify-center shrink-0"><MessageSquarePlus size={17} className="text-orange" /></div>
            <div>
          ...
          <button onClick={() => { fetchBlockedUsers(); setShowBlocked(true) }} className="w-full flex items-center gap-3 bg-white border border-border rounded-card px-4 py-4 text-left active:scale-[0.98] transition-transform">
            <div className="w-9 h-9 rounded-xl bg-danger/10 flex items-center justify-center shrink-0"><UserX size={17} className="text-danger" /></div>
            <div>
        ...
        <p className="text-center text-[10px] text-muted pb-4">Senior Se Le · Beta v1.0 · Made with ❤️ for students</p>
      </div>
