...
// editBook: pass an existing book row to switch this form into Edit mode
export default function PostBook({ editBook = null, onBack, onPosted }) {
  const { user } = useAuth()
  const isEditMode = !!editBook

  // Images: existing ones (URLs, kept unless removed) + new ones (files, to be uploaded)
  const [existingImages, setExistingImages] = useState(editBook?.images || [])
  const [newImages, setNewImages] = useState([])

  // Video: existing url (kept unless removed/replaced) + new file
  ...
  function handleVideoPick(e) {
    ...
    setExistingVideoUrl(null) // replacing
  ...
  async function handleSubmit(e) {
    ...
    if (totalImageCount === 0) {
      setError('Add at least one photo of the book')
      return
    }
    if (!title || !price) {
    ...
    if (originalPrice && Number(price) >= Number(originalPrice)) {
      setError('Your selling price must be less than the original price — this is a resale platform')
      return
    }
    ...
    try {
      ...
      // Upload new video if present, otherwise keep existing url (or null if removed)
    ...
    <div className="min-h-screen bg-cream pb-28">
      ...
      <form onSubmit={handleSubmit} className="px-4 pt-5 flex flex-col gap-5">
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {existingImages.map((url, idx) => (
              <div key={`existing-${idx}`} className="relative aspect-square rounded-button overflow-hidden border border-border">
                ...
                {idx === 0 && (
                  <span className="absolute bottom-1 left-1 bg-orange text-white text-[8px] font-bold px-1.5 py-0.5 rounded">
                    COVER
                  </span>
                )}
              </div>
            ...
            {newImages.map((img, idx) => (
              <div key={`new-${idx}`} className="relative aspect-square rounded-button overflow-hidden border border-border">
        ...
        <div>
          ...
          {!hasVideo ? (
            <label
              className={`flex flex-col items-center justify-center gap-2 rounded-card border-2 border-dashed py-6 cursor-pointer bg-white ${
                videoRequired ? 'border-orange' : 'border-border'
              }`}
            >
            ...
            <div className="relative rounded-card overflow-hidden border border-border bg-black">
              <video
                src={newVideo?.preview || existingVideoUrl}
          ...
          <div className="flex items-start gap-2 bg-success-bg rounded-button px-3.5 py-3 mt-2.5">
            ...
            <p className="text-[11px] text-navy leading-relaxed">
              <span className="font-semibold">Books with video get a "Verified Condition" badge</span> and
              sell up to 2x faster — buyers trust what they can see moving. A quick 15-second pan of the
              cover, spine, and pages builds instant confidence.
            </p>
          </div>
        </div>
            ...
            {CATEGORIES.map((cat) => (
              <button
                ...
                className={`text-xs font-semibold font-display px-3.5 py-2 rounded-chip transition-colors ${
                  category === cat ? 'bg-orange text-white' : 'bg-white text-navy border border-border'
                }`}
              >
                {cat}
              </button>
        ...
        <div>
          ...
          <div className="bg-white rounded-card border border-border p-4">
            <div className="flex justify-between mb-3">
              {[1, 2, 3, 4, 5].map((level) => (
                <button
                  ...
                  className={`w-9 h-9 rounded-full text-xs font-bold font-display transition-all ${
                    condition === level
            ...
            <p className="text-center text-sm font-semibold text-navy font-display">
              {CONDITIONS[condition].label}
            </p>
            <p className="text-center text-[11px] text-muted mt-1">
              Be honest — mismatched condition leads to disputes and refunds
            </p>
          </div>
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Any highlighting, missing pages, or notes for the buyer…"
            rows={3}
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Input
            ...
            error={
              originalPrice && price && Number(price) >= Number(originalPrice)
                ? 'Must be higher than your selling price'
                : ''
            }
          />
        </div>
          ...
          <div className="bg-navy/5 rounded-card p-4 flex flex-col gap-2">
            ...
            <div className="flex justify-between text-xs items-center">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-bold text-success bg-success-bg px-2 py-0.5 rounded-chip">
                  Waived during beta 🎉
                </span>
              </div>
            </div>
