...
export default function BookDetail({ book, onBack, onChat, onReport, onEdit, onRemoved }) {
  const { user } = useAuth()
  const [activeMedia, setActiveMedia] = useState(0) // index into media array (images + video)
    ...
    <div className="min-h-screen bg-cream pb-28">
      ...
      <div className="relative bg-navy/5 aspect-square">
        {media.length > 0 ? (
          media[activeMedia]?.video ? (
            ...
            <img src={media[activeMedia]} alt={book.title} className="w-full h-full object-contain px-3" />
          )
        ) : (
        ...
        <div className="flex gap-2 px-4 pt-3 overflow-x-auto scrollbar-hide">
          {media.map((m, idx) => (
            <button
              ...
              className={`shrink-0 w-14 h-14 rounded-button overflow-hidden border-2 ${
                activeMedia === idx ? 'border-orange' : 'border-border'
              }`}
            >
              {m?.video ? (
      ...
      <div className="px-4 pt-5 flex flex-col gap-5">
        ...
        <div className="flex gap-2 flex-wrap">
          {book.category && (
            <span className="text-xs font-semibold text-navy bg-white border border-border px-3 py-1.5 rounded-chip">
              {book.category}
            </span>
          )}
          {book.subject && (
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-cream/95 backdrop-blur border-t border-border px-4 py-4">
        {isOwner ? (
          <div className="flex gap-3">
            <Button variant="subtle" size="lg" className="flex-1" icon={Edit2} onClick={() => onEdit?.(book)}>
              Edit
            </Button>
            <Button
              variant="danger"
